...
if not "%CD:~60%"=="" (
    ...
    echo [!] 지금 폴더 위치가 너무 깊습니다: %CD%

    echo.

    echo     Windows는 전체 경로가 너무 길면 설치 도중 알 수 없는 오류를 냅니다.

    echo     아래처럼 짧은 위치로 옮긴 뒤 그 안에서 다시 실행해주세요.

    echo.

    echo     1^) 지금 이 "naver_blog_manager" 폴더 전체를 잘라내기^(Cut^)

    echo     2^) C:\ 드라이브 바로 밑에 붙여넣기 ^(예: C:\naver_blog_manager^)

    echo     3^) 그 폴더 안에서 1-install.bat 을 다시 더블클릭
...
echo   이제부터는 "2-start.bat" 파일만 더블클릭하면 됩니다.
...
rem Look for a Python that can actually run code, not just a PATH entry.

rem The fake MS Store shortcut shows up in "where" but fails when executed.
