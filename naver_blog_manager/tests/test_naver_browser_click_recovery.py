...
클릭이 막혔고, 그때마다 선택자 기반 팝업닫기 시도(_try_dismiss_resume_draft_dialog,
_try_close_popup)가 실제 팝업의 class/구조와 안 맞아 실패했다 - 결국 사용자가 직접
"취소"를 눌러야 했다. 네이버 내부 DOM은 공개 문서가 없어 선택자를 계속 추측하는 건
비효율적이므로, 뭐가 됐든(어떤 팝업이든) 통하는 선택자 무관 방식으로 바꿨다: 일반
