...
def _keyword_summary(db: Session, keyword: models.Keyword) -> dict:
    latest_check = (
...
def get_summary(db: Session = Depends(get_db)):
    keywords = (
        ...
        .order_by(models.Keyword.created_at.asc())
        .all()
    )
    summaries = [_keyword_summary(db, k) for k in keywords]
    open_alert_count = sum(1 for s in summaries if s["has_open_alert"])
    stats = dashboard_service.aggregate_stats(summaries, open_alert_count)
