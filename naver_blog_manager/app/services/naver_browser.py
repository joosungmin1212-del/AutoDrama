...
async def login_interactive(timeout_ms: int = 180_000) -> bool:
    """실제 브라우저 창을 열어 사용자가 직접 로그인하도록 하고, 세션을 저장한다.
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await context.storage_state(path=str(config.NAVER_STATE_PATH))
            return True
        finally:
            await browser.close()
    ...
    async with async_playwright() as p:
        ...
        try:
            context = await browser.new_context(
                storage_state=str(config.NAVER_STATE_PATH),
...
async def open_write_draft(blog_id: str, title: str, content_html: str) -> None:
    ...
    사용자가 검토/발행할 수 있도록 열린 채로 남겨둔다(자동으로 닫지 않음).
    """
    if not has_saved_session():
    ...
    # 브라우저/컨텍스트/플레이라이트를 의도적으로 종료하지 않음 - 사용자가 검토 후 발행하도록 둔다.
