...
def apply_content_matches(db: Session, items: list[RankItem], watch_names: list[str]) -> None:
    ...
    "우리 것 아님"으로 확정돼 있어야 하는 대상이기 때문이다:
      1. 경쟁업체로 등록해둔 블로그(item.matched_blog.role == "competitor") - 주변 다른
         트레이너/PT샵 본인 블로그는 자기 홍보가 목적이라 우리 얘기가 나올 일이 사실상
         없다. 등록해두면 계정 단위로 영구히 "확인된 타업체"로 남는다.
         (체험단은 다르다 - item.matched_blog가 있어도(체험단으로 등록해둔 계정)
         건너뛰지 않는다. 같은 체험단 블로거가 이 키워드에는 우리 글을, 다른 키워드에는
         완전히 다른 업체 글을 쓰는 경우가 흔해서, 계정 단위로 못 박지 않고 글(URL)마다
         따로 판정해야 한다.)
    ...
    for item in items:
        if item.ownership != models.Ownership.OTHER.value:
            continue
        if getattr(item.matched_blog, "role", None) == models.BlogRole.COMPETITOR.value:
            continue
