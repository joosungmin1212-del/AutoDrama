"""
Phase 1: Outline 생성 프롬프트 (V2 - Enhanced)
72B 모델 최적화 버전
"""
...
4. 파트별 감정 강도 설계
   - Part 1-4의 감정 강도 곡선 (1-10 스케일)
   - 각 파트의 지배 감정(dominant emotion) 명시
...
{{
  ...
  "characters": [
    {{
      ...
      "key_motivation": "이 인물이 원하는 것 (구체적으로)"
    }}
  ],

  "story_spine": {{
    "act1_setup": {{
      "duration": "Part 1 전반 (0-15분)",
      "key_events": [
    ...
    "act2_conflict": {{
      "duration": "Part 1 후반 ~ Part 2 (15-60분)",
      "key_events": [
        "갈등 심화 사건들",
        "과거 회상 장면",
        "가족 간 의견 충돌"
    ...
    "act3_midpoint": {{
      "duration": "Part 2 후반 ~ Part 3 전반 (60-90분)",
  ...
  "emotional_beats": [
    {{
      "part": 1,
      "dominant_emotion": "호기심|그리움|불안 중 택1",
      "intensity": 6,
      "key_moments": [
        "순간 1: 반지 발견 시 놀라움",
        "순간 2: 가족 반응에 당황"
      ],
      "ending_emotion": "Part 1 끝 감정"
    }},
    {{
      "part": 2,
      "dominant_emotion": "갈등|혼란|고통",
      "intensity": 7,
      "key_moments": [
        "순간 1: 가족과 충돌",
        "순간 2: 과거 회상 중 눈물"
      ],
      "ending_emotion": "Part 2 끝 감정"
    }},
    {{
      "part": 3,
      "dominant_emotion": "절정|깨달음|결단",
      "intensity": 9,
      "key_moments": [
        "순간 1: 진실 발견",
        "순간 2: 최종 선택의 순간"
      ],
      "ending_emotion": "Part 3 끝 감정"
    }},
    {{
      "part": 4,
      "dominant_emotion": "화해|평온|희망",
      "intensity": 5,
      "key_moments": [
        "순간 1: 용서의 장면",
        "순간 2: 새로운 시작"
      ],
      "ending_emotion": "평온함|따뜻한 여운"
    }}
  ],
  ...
  "key_scenes": [
    {{
      ...
      "sensory_details": {{
        ...
        "sight": "낡았지만 빛나는 금빛",
        "smell": "세제 향기",
        "emotional_trigger": "심장이 멎는 듯한 순간"
      }},
  ...
  "part_breakdown": [
    {{
      ...
      "title": "발견 (Discovery)",
      "time_range": "0-30분",
      "word_count": "12000-13000자",

      "primary_goal": "상황 제시, 인물 소개, 갈등 씨앗",
      "conflict_level": 3,
      "dominant_emotion": "호기심/그리움",
      ...
      "bridge_to_next": {{
        "connector": "하영의 질문으로 Part 2 시작 암시",
        "carry_over": ["반지", "가족 우려", "유진의 고민"]
      }}
    }},
    ...
    {{
      ...
      "bridge_to_next": {{
        "connector": "충격적 발견으로 Part 3 시작",
        "carry_over": ["진실 단서", "유진의 혼란", "가족 긴장"]
      }}
    }},
    ...
    {{
      ...
      "bridge_to_next": {{
        "connector": "결단 후 변화 시작",
        "carry_over": ["선택의 여파", "관계 변화", "새로운 균형"]
      }}
    }},
    ...
    {{
      ...
      "title": "해소 (Resolution)",
      "time_range": "105-120분",
      "word_count": "11500-12500자",

      "primary_goal": "갈등 해소, 화해, 새로운 시작",
      "conflict_level": 4,
      "dominant_emotion": "평온/희망",
  ...
  "outline_full": "위 모든 요소를 통합한 전체 스토리 서술 (3000-3500자).

  이 서술은 단순 요약이 아니라, 대본 작가가 읽고 전체 흐름을 완벽히 이해할 수 있도록:
  - 사건의 인과관계 명확히
  - 감정의 변화 과정 상세히
  - 캐릭터 동기와 선택 논리적으로
  - 테마의 전개 자연스럽게

  서술하세요. 시간 순서대로, 구체적으로, 감정선을 따라가며 작성하세요."
}}
...
2. 캐릭터 입체성
   - 단순한 역할이 아닌, 살아있는 인물로 설계하세요
   - 각 인물의 욕망, 두려움, 변화를 명확히 하세요
   - 인물 간 관계가 스토리를 이끌도록 하세요
...
def generate_outline_prompt_v2(title: str) -> str:
    """
    Outline 생성 프롬프트 V2를 생성합니다.
    ...
    Returns:
        str: 완성된 프롬프트
    """
    return OUTLINE_PROMPT_V2.format(title=title)
